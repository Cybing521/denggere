...
# PySR disabled on this machine (Julia backend hangs during init)
...
N_H = 1e6                 # effective contact population
DAYS_PER_MONTH = 30
...
class SEIRCoupledModel(nn.Module):
    ...
    The NN predicts β_norm ∈ (0,1), linearly scaled:
        β'(t) = s1 * MLP(weather) + s0
    as described in the thesis (Eq. 5).
    ...
    def __init__(self, n_hidden: int = 16, n_h: float = N_H):
        ...
        # Linear scaling: β' = s1 * sigmoid_output + s0
        self.s1 = nn.Parameter(torch.tensor(0.3))   # scale
        self.s0 = nn.Parameter(torch.tensor(0.01))   # offset
        self.log_eta = nn.Parameter(torch.tensor(0.0))  # η = exp(log_eta), init ~1.0 person/day
        self.n_h = n_h

    @property
    def beta_scale(self) -> torch.Tensor:
        """For backward compat with logging."""
        return self.s1
    ...
    def forward(
        ...
        m_norm: torch.Tensor,
        year_ids: torch.Tensor = None,
        days: int = DAYS_PER_MONTH,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Forward pass: NN → β → SEIR ODE → predicted monthly cases.

        SEIR state resets at the start of each year.
        Returns (pred_cases, beta) where beta is the physical-scale β'(t).
        """
        beta_norm = self.nn_beta(weather_norm).squeeze(-1)  # (T,) in (0,1)
        beta = torch.clamp(self.s1 * beta_norm + self.s0, min=1e-6)
        eta = self.eta
        n_h = self.n_h
        ...
        S, E, I, R = _init_state()
        prev_year = int(year_ids[0].item()) if year_ids is not None else None

        cases_list: List[torch.Tensor] = []
        for t in range(T):
            if year_ids is not None:
                cur_year = int(year_ids[t].item())
...
def seir_forward(
    ...
    n_h: float = N_H,
    days: int = DAYS_PER_MONTH,
    year_ids: np.ndarray = None,
) -> np.ndarray:
    """Non-differentiable SEIR forward simulation (numpy)."""
    T = len(beta)
    S, E, I, R = n_h - 1.0, 0.0, 1.0, 0.0
    prev_year = int(year_ids[0]) if year_ids is not None else None
    cases_out = np.zeros(T)
    for t in range(T):
        if year_ids is not None:
            cur_year = int(year_ids[t])
            if cur_year != prev_year:
                S, E, I, R = n_h - 1.0, 0.0, 1.0, 0.0
                prev_year = cur_year
        mc = 0.0
...
def attach_mosquito_proxy(city_df: pd.DataFrame, bi_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Normalize to [0, 1] as per thesis Eq. (2): M_hat = BI / max(BI)
    bi_max = out["index_norm_city"].max()
    if bi_max > 0:
        out["index_norm_city"] = out["index_norm_city"] / bi_max
    out["index_norm_city"] = np.clip(out["index_norm_city"], 0.01, 1.0)
    return out
...
def train_coupled_model(
    ...
    y_ids = torch.tensor(year_ids_np, dtype=torch.int64)
    ...
    for epoch in range(n_epochs):
        ...
        pred_cases, _ = model(w_t, m_t, year_ids=y_ids)

        p = pred_cases[mask]
        o = c_t[mask]

        loss_mse = torch.mean((torch.log1p(p) - torch.log1p(o)) ** 2)
        # Also add raw MSE to help capture peak magnitudes
        loss_raw = torch.mean((p - o) ** 2) / (o.std() ** 2 + 1.0)
        ...
        loss = loss_mse + loss_raw + alpha * loss_corr
        loss.backward()
...
# ── Symbolic regression / polynomial fallback ────────────────────────────

class PolyFormulaModel:
    """Quadratic polynomial with interaction terms: β = f(T, H, R).

    This is the same formula family as the thesis describes (Eq. 8):
    β' = max(0, a0 + aT*T + aH*H + aR*R + aTT*T² + aHH*H² + aRR*R²
             + aTH*T*H + aTR*T*R + aHR*H*R)
    """
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "PolyFormulaModel":
        X_poly = self.poly.fit_transform(X)
        self._feature_names = self.poly.get_feature_names_out(["T", "H", "R"]).tolist()
        self.reg.fit(X_poly, y)
        return self

    def predict(self, X: np.ndarray) -> np.ndarray:
        X_poly = self.poly.transform(X)
        return np.maximum(self.reg.predict(X_poly), 0.0)

    def get_coefficients(self) -> Dict[str, float]:
        return dict(zip(self._feature_names, self.reg.coef_.tolist()))

    def equation_str(self) -> str:
        parts = []
        for name, coef in zip(self._feature_names, self.reg.coef_):
            if abs(coef) < 1e-15:
                continue
            if name == "1":
                parts.append(f"{coef:.6e}")
            else:
                parts.append(f"{coef:.6e}*{name}")
...
def fit_formula(
    ...
    """Fit symbolic formula. Uses PySR if available, else polynomial fallback.

    Returns (model, metrics_dict, equation_string).
    """
    if _PYSR_AVAILABLE:
        print("  Using PySR symbolic regression ...")
        sr = PySRRegressor(
            binary_operators=["+", "-", "*", "/"],
        ...
        print("  Using polynomial regression (degree=2 with interactions) ...")
        sr = PolyFormulaModel().fit(weather_raw, beta)
        pred = sr.predict(weather_raw)
        eq_str = sr.equation_str()
    ...
    return sr, met, eq_str


def predict_with_formula(model: object, weather_raw: np.ndarray) -> np.ndarray:
    """Predict β using either PySR or PolyFormulaModel."""
    if isinstance(model, PolyFormulaModel):
        return model.predict(weather_raw)
    else:
...
def predict_city_risk_monthly(
    ...
    formula_model: object,
    eta: float,
    n_h: float = N_H,
) -> pd.DataFrame:
    """Predict monthly cases for a city using formula + SEIR forward."""
    weather = city_df[["tem", "rhu", "pre"]].values.astype(float)
    beta = predict_with_formula(formula_model, weather)
    m_norm = city_df["index_norm_city"].values.astype(float)
    pred_cases = seir_forward(beta, m_norm, eta, n_h, year_ids=city_df["year"].values.astype(int))
...
def plot_outbreak_2014_beta(gz: pd.DataFrame) -> None:
    """Compare 2014 monthly β with other years' monthly β."""
    gz = gz.copy()
    gz["month_idx"] = gz["month"].astype(int)
    is2014 = gz["year"] == 2014
    other = gz[~is2014].groupby("month_idx")["beta_learned"].agg(["mean", "std"]).reset_index()
    y2014 = gz[is2014][["month_idx", "beta_learned"]].sort_values("month_idx")

    fig, ax = plt.subplots(figsize=(7, 4.5))
    ax.fill_between(
        other["month_idx"],
        other["mean"] - other["std"],
        other["mean"] + other["std"],
        alpha=0.25, color="steelblue", label="Other years (mean ± std)",
    )
    ax.plot(other["month_idx"], other["mean"], "b-", lw=1.5)
    ax.plot(y2014["month_idx"], y2014["beta_learned"], "r-o", lw=2, ms=5, label="2014")
    ax.set_xlabel("Month")
    ax.set_ylabel("β' (day⁻¹)")
    ax.set_title("Monthly β': 2014 vs other years")
    ax.legend(fontsize=9)
    ax.grid(alpha=0.25)
    ax.set_xticks(range(1, 13))
    fig.tight_layout()
    fig.savefig(OUT / "outbreak_2014_beta_compare_data2.png", dpi=220)
    plt.close(fig)


def plot_all_cities_grid(transfer_monthly: pd.DataFrame) -> None:
    """4×4 grid of monthly observed vs predicted for all 16 cities."""
    cities = sorted(transfer_monthly["city_en"].unique())
    n = len(cities)
    ncols = 4
    ...
    for i, city in enumerate(cities):
        ax = axes[i]
        cdf = transfer_monthly[transfer_monthly["city_en"] == city].sort_values(["year", "month"])
        t = np.arange(len(cdf))
        ax.plot(t, cdf["cases"].values, "k-", lw=1.2, label="Obs")
        ax.plot(t, cdf["risk_monthly"].values, "r-", lw=0.9, alpha=0.8, label="Pred")
    ...
    fig.suptitle("16 cities: monthly observed vs formula-predicted (2005-2019)", fontsize=11)
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    fig.savefig(OUT / "all_cities_fit_grid.png", dpi=180)
    plt.close(fig)


def plot_per_city_curves(transfer_monthly: pd.DataFrame) -> None:
    """Individual per-city fit curves saved to city_fit_curves/ subfolder."""
    city_dir = OUT / "city_fit_curves"
    ...
    for city in sorted(transfer_monthly["city_en"].unique()):
        ...
        pred = cdf["risk_monthly"].values.astype(float)
        ax.plot(t, obs, "k-", lw=1.5, label="Observed")
        ax.plot(t, pred, "r-", lw=1.1, alpha=0.8, label="Predicted")
        ax.set_yscale("symlog", linthresh=1.0)
        # Compute metrics
        if np.std(obs) > 0 and np.std(pred) > 0:
            r_val = pearsonr(obs, pred)[0]
            rho_val = spearmanr(obs, pred)[0]
        else:
            r_val = rho_val = np.nan
        ax.set_title(f"{city}  (r={r_val:.3f}, ρ={rho_val:.3f})", fontsize=10)
        ax.set_xlabel("Month index")
        ax.set_ylabel("Cases (symlog)")
        ax.grid(alpha=0.25)
        ax.legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(city_dir / f"fit_{city}.png", dpi=180)
        plt.close(fig)
...
def main() -> None:
    ...
    print("Phase 1: Training NN-coupled SEIR model on Guangzhou ...", flush=True)
    ...
    year_ids = gz["year"].values.astype(int)
    model, losses = train_coupled_model(x_norm, m_norm_gz, cases_obs, train_mask, year_ids)

    with torch.no_grad():
        pred_cases_t, beta_t = model(
            torch.tensor(x_norm, dtype=torch.float32),
            torch.tensor(m_norm_gz, dtype=torch.float32),
            year_ids=torch.tensor(year_ids, dtype=torch.int64),
        )
    ...
    print("Phase 2: Running symbolic regression / polynomial fit ...")
    sr_model, f_metrics, eq_str = fit_formula(weather_raw, beta_learned)
    beta_formula_gz = predict_with_formula(sr_model, weather_raw)

    pd.DataFrame([f_metrics]).to_csv(OUT / "phase2_formula_fit_metrics_data2.csv", index=False)

    # Save coefficients if polynomial model
    if isinstance(sr_model, PolyFormulaModel):
        coef_dict = sr_model.get_coefficients()
        pd.DataFrame([coef_dict]).to_csv(OUT / "phase2_poly_coefficients.csv", index=False)
    elif _PYSR_AVAILABLE:
    ...
    plot_phase2_formula(beta_learned, beta_formula_gz, equation_str=eq_str)
    plot_transfer(annual2014)
    plot_outbreak_2014_beta(gz)
    plot_all_cities_grid(transfer_monthly)
    plot_per_city_curves(transfer_monthly)

    # ── Compute per-city monthly metrics ─────────────────────────────
    city_monthly_metrics = []
    for city in sorted(transfer_monthly["city_en"].unique()):
        cdf = transfer_monthly[transfer_monthly["city_en"] == city]
        obs = cdf["cases"].values.astype(float)
        pred = cdf["risk_monthly"].values.astype(float)
        m = evaluate_cases(obs, pred)
        m["city_en"] = city
        city_monthly_metrics.append(m)
    city_monthly_df = pd.DataFrame(city_monthly_metrics)
    city_monthly_df.to_csv(OUT / "transfer_city_monthly_metrics.csv", index=False)
    print("\nPer-city monthly metrics:")
    print(city_monthly_df[["city_en", "pearson_r", "spearman_rho", "r2_log", "mae", "rmse"]].to_string(index=False))
