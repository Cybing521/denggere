...
The study is organized into three parts. Part~I uses 8~cities with Breteau Index (BI) monitoring data (306~samples) to discover the mosquito density formula $\hat{M} = \cos(T/T_m - 1.87)$ through neural network + PySR symbolic regression, with leave-one-city-out cross-validation (LOCO~CV) mean $R^2=0.144$.

Part~II focuses on Guangzhou (2005--2019 monthly data), employing the Bri\`{e}re function to model the temperature dependence of transmission rate. Differential evolution optimization yields optimal parameters ($T_{\min}=15.1^\circ$C, $T_{\max}=42.0^\circ$C, $T_{\text{opt}}=35.5^\circ$C). Guangzhou fitting achieves Spearman~$\rho=0.814$ and log-scale $R^2=0.851$; leave-one-year-out cross-validation (LOYO~CV) for outbreak years yields mean $\rho=0.806$. Transfer to 16~cities via shared Bri\`{e}re parameters + per-city $\eta$ calibration achieves monthly mean $\rho=0.531$, $R^2_{\log}=0.656$, WAPE${}=0.827$; 2014 annual ranking $\rho=0.947$.

Part~III performs independent temporal-external validation using new Guangzhou BI monitoring data from 2020--2026. The Bri\`{e}re $\beta'(T)$ correlates with observed BI at Pearson~$r=0.782$ and seasonal $r=0.920$, confirming temporal generalizability. A purely data-driven neural network $\beta$ approach is presented as a comparative experiment, revealing mode collapse ($\beta_{\text{NN}}$ coefficient of variation = 1.17\%) under 180 monthly samples with 64\% low/zero-case months, highlighting the necessity of physics-informed approaches for small-sample infectious disease modeling.
