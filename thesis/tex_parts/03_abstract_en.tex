...
Part~II centers on Guangzhou (2005--2019 monthly data), employing the Bri\`{e}re function $\beta'(T) = cT(T-T_{\min})\sqrt{T_{\max}-T}$ to model the temperature dependence of the transmission rate, coupled with an SEIR compartmental model for case prediction. Differential evolution optimization yields optimal parameters ($T_{\min}=14.9^\circ$C, $T_{\max}=42.0^\circ$C, $T_{\text{opt}}=35.5^\circ$C), with Guangzhou fit Spearman~$\rho=0.813$ and log-scale $R^2=0.855$; leave-one-year-out CV (LOYO~CV) outbreak-year mean $\rho=0.798$. The model is then transferred to 16~cities via shared Bri\`{e}re parameters + per-city $\eta$ calibration, achieving monthly mean $\rho=0.531$, $R^2_{\log}=0.656$, and 2014 annual ranking $\rho=0.938$. Independent temporal-external validation using 2020--2026 Guangzhou BI data yields Bri\`{e}re $\beta'(T)$ vs.\ observed BI Pearson~$r=0.782$ and seasonal $r=0.919$, confirming temporal generalizability.
