...
# ── Multi-city joint optimization ─────────────────────────────────────

def objective_multi(params, city_data_list, train_year_exclude=2014):
    """Joint objective: shared Brière (c, T_min, T_max) + per-city eta.

    params = [c, T_min, T_max, eta_0, eta_1, ..., eta_{N-1}]
    """
    n_cities = len(city_data_list)
    c, T_min, T_max = params[0], params[1], params[2]
    etas = params[3:3 + n_cities]
    ...
    total_loss = 0.0
    n_valid = 0
    for i, (tem, cases_obs, m_hat, N_h, yrs) in enumerate(city_data_list):
        eta = etas[i]
        ...
        weight = np.sqrt(np.sum(obs_tr) + 1.0)
        total_loss += weight * (loss_main + 0.3 * loss_corr)
        n_valid += 1
...
def plot_all_cities_grid(tm, path):
    ...
    for i, city in enumerate(cities):
        ...
        t = np.arange(len(cdf))
        obs_vals = cdf["cases"].values
        pred_vals = cdf["pred_cases"].values

        # For low-incidence cities: clip tiny predictions to 0 for cleaner display
        city_max_obs = np.max(obs_vals)
        if city_max_obs < 20:
            # Low-incidence: use linear scale, clip noise
            noise_thresh = max(city_max_obs * 0.05, 0.5)
            pred_display = np.where(pred_vals < noise_thresh, 0, pred_vals)
            ax.plot(t, obs_vals, "k-", lw=1.2, label="Obs")
            ax.plot(t, pred_display, "r-", lw=0.9, alpha=0.8, label="Pred")
            ax.set_ylim(-0.5, max(city_max_obs * 1.3, 5))
        else:
            ax.plot(t, obs_vals, "k-", lw=1.2, label="Obs")
            ax.plot(t, pred_vals, "r-", lw=0.9, alpha=0.8, label="Pred")
            ax.set_yscale("symlog", linthresh=1)

        ax.set_title(city, fontsize=9); ax.grid(alpha=0.2)
...
def main():
    ...
    # Stage 1: Fit shared Brière (c, T_min, T_max) + eta on Guangzhou
    # Stage 2: Fix Brière, fit per-city eta for each city
    print("\nStage 1: Optimizing Brière on Guangzhou ...")
    ...
    for city in all_cities:
        ...
        city_data[city] = {
            "df": cdf, "tem": cdf["tem"].values.astype(np.float64),
            "cases": cdf["cases"].values.astype(np.float64),
            "m_hat_norm": m_hat_norm, "m_hat_bi": m_hat_bi,
            "N_h": N_h, "years": cdf["year"].values.astype(int),
        }

    gz_d = city_data["Guangzhou"]
    tem_gz = gz_d["tem"]
    cases_gz = gz_d["cases"]
    m_hat_norm_gz = gz_d["m_hat_norm"]
    years_gz = gz_d["years"]
    train_mask_gz = years_gz != 2014

    bounds_single = [
        (1e-8, 1e-2), (10.0, 20.0), (32.0, 42.0), (0.01, 50.0),
    ]
    ...
    T_opt_arr = np.linspace(T_min_opt, T_max_opt, 1000)
    T_opt_val = T_opt_arr[np.argmax(briere(T_opt_arr, c_opt, T_min_opt, T_max_opt))]
    print(f"  Brière: c={c_opt:.2e}, T_min={T_min_opt:.1f}, T_max={T_max_opt:.1f}, T_opt={T_opt_val:.1f}")
    print(f"  GZ eta={eta_gz:.4f}")

    # Stage 2: Fix Brière, fit per-city eta via grid search
    print("\nStage 2: Fitting per-city eta (fixed Brière) ...")
    city_etas = {}
    eta_candidates = np.logspace(-2, np.log10(50), 80)
    for city in all_cities:
        cd = city_data[city]
        non2014 = cd["years"] != 2014
        best_loss, best_eta = 1e12, 1.0
        for eta_try in eta_candidates:
            try:
                ...
                continue
        city_etas[city] = best_eta

    print(f"  Per-city eta range: [{min(city_etas.values()):.4f}, {max(city_etas.values()):.4f}]")
    for city in all_cities:
        print(f"    {city:12s}: eta={city_etas[city]:.4f}")
    ...
    print(f"\n  GZ train:  ρ={gz_met_train['spearman_rho']:.3f}, R²_log={gz_met_train['r2_log']:.3f}")
    ...
    # Save per-city etas
    eta_df = pd.DataFrame([{"city_en": c, "eta": city_etas[c]} for c in all_cities])
    eta_df.to_csv(OUT / "per_city_etas.csv", index=False)

    # ── Leave-One-Year-Out CV on Guangzhou (single-city objective) ────
    print("\nLeave-One-Year-Out CV on Guangzhou ...")
    bounds_single = [
        (1e-8, 1e-2), (10.0, 20.0), (32.0, 42.0), (0.01, 50.0),
    ]
    cv_rows = []
    for test_year in sorted(np.unique(years_gz)):
        ...
        if cv_test_mask.sum() > 0:
            ...
            met["year"] = test_year
            # Add outbreak detection metric
            obs_y = cases_gz[cv_test_mask]
            pred_y = cv_pred[cv_test_mask]
            met["obs_total"] = float(np.sum(obs_y))
            met["obs_max"] = float(np.max(obs_y))
            met["is_outbreak"] = float(np.sum(obs_y) > 50)
            cv_rows.append(met)
            tag = "✓" if met["r2_log"] > 0.3 else ("~" if met["r2_log"] > 0 else "✗")
            print(f"  {test_year}: ρ={met['spearman_rho']:.3f}, R²_log={met['r2_log']:.3f} {tag}  "
                  f"(cases={met['obs_total']:.0f})")

    cv_df = pd.DataFrame(cv_rows)
    cv_df.to_csv(OUT / "loyo_cv.csv", index=False)

    # Report separately for outbreak vs non-outbreak years
    outbreak_mask = cv_df["is_outbreak"] == 1.0
    if outbreak_mask.sum() > 0:
        cv_outbreak = cv_df[outbreak_mask]
        cv_quiet = cv_df[~outbreak_mask]
        print(f"  LOYO outbreak years (n={len(cv_outbreak)}): "
              f"ρ={cv_outbreak['spearman_rho'].mean():.3f} ± {cv_outbreak['spearman_rho'].std():.3f}")
        if len(cv_quiet) > 0:
            print(f"  LOYO quiet years   (n={len(cv_quiet)}): "
                  f"ρ={cv_quiet['spearman_rho'].mean():.3f} ± {cv_quiet['spearman_rho'].std():.3f}")
    print(f"  LOYO all years: ρ={cv_df['spearman_rho'].mean():.3f} ± {cv_df['spearman_rho'].std():.3f}")

    # ── Per-city predictions with per-city eta + per-city scaling ─────
    print("\nGenerating per-city predictions (joint Brière + per-city eta + scaling) ...")
    ...
    for city in all_cities:
        ...
        non2014 = cd["years"] != 2014
        obs_train = cd["cases"][non2014]
        pred_train = pred_raw[non2014]

        # Log-space ratio scaling (robust)
        obs_m = np.mean(obs_train) + 1e-3
        pred_m = np.mean(pred_train) + 1e-3
        scale = obs_m / pred_m
        pred_scaled = np.maximum(pred_raw * scale, 0.0)

        cdf = cd["df"].copy()
        cdf["m_hat_bi"] = cd["m_hat_bi"]
        cdf["m_hat_norm"] = cd["m_hat_norm"]
        cdf["pred_raw"] = pred_raw
        cdf["pred_cases"] = pred_scaled
        cdf["beta_prime"] = briere(cd["tem"], c_opt, T_min_opt, T_max_opt)
        cdf["eta"] = eta_c
        cdf["scale_factor"] = scale
        city_tables.append(cdf)

        met = evaluate_cases(cd["cases"], pred_scaled)
        met["city_en"] = city
